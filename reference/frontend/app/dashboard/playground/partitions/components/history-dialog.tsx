...
import {
  usePartitionList,
  useDeletePartition,
} from "@/app/dashboard/widgets/queries/partitions";
...
export const PartitionHistoryDialog = ({
  open,
  onOpenChange,
  onSelectPartition,
}: {
  ...
  onSelectPartition?: (partition: StoredPartition) => void;
}) => {
  // Pagination state
  const [partitionsBefore, setPartitionsBefore] = useState<string | null>(null);
  const [partitionsAfter, setPartitionsAfter] = useState<string | null>(null);
  ...
  // Delete state. UI signaling only: hide the delete affordance when the org
  // RBAC capability is absent. The backend route remains the security boundary.
  const canDeletePartition = useCanOrganization("rbac:partition:delete");
  const { mutateAsync: deletePartition } = useDeletePartition();
  ...
  const handleSearchChange = (value: string) => {
    ...
    setPartitionsBefore(null);
    setPartitionsAfter(null);
  };

  // Query partitions — refetch on mount keeps results fresh after re-opening
  const partitionsQuery = usePartitionList(
    {
      before: partitionsBefore || undefined,
      after: partitionsAfter || undefined,
      limit: limit,
  ...
  const partitionsData = partitionsQuery.data;
  const partitions = partitionsData?.data || [];
  const partitionsMetadata = partitionsData?.list_metadata;
  const refetchPartitions = partitionsQuery.refetch;
  ...
  const partitionsNextPage = useCallback(() => {
    if (partitionsMetadata?.after) {
      setPartitionsAfter(partitionsMetadata.after);
      setPartitionsBefore(null);
    }
  }, [partitionsMetadata]);

  const partitionsPrevPage = useCallback(() => {
    if (partitionsMetadata?.before) {
      setPartitionsBefore(partitionsMetadata.before);
      setPartitionsAfter(null);
    }
  }, [partitionsMetadata]);
  ...
  const handlePartitionClick = useCallback(
    ...
    [onSelectPartition, onOpenChange],
  );
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deletePartition(deleteConfirmId);
  ...
  return (
    <>
      {open && <RefetchOnOpen refetch={refetchPartitions} />}
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="flex max-h-[80vh] min-h-[80vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl">
          <DialogHeader className="px-6 pt-6 pb-4">
            <DialogTitle>Partition History</DialogTitle>
            <DialogDescription className="sr-only">
              View and load your previous partition results
            </DialogDescription>
          </DialogHeader>
              ...
              <Table className="w-full table-fixed">
                ...
                <TableHeader className="sticky top-0 z-10 bg-white">
                  <TableRow className="border-none">
                    ...
                    <TableHead className="text-xs text-black">Key</TableHead>
                    <TableHead className="text-xs text-black">Pages</TableHead>
                    <TableHead className="text-xs text-black">Chunks</TableHead>
              ...
              <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-4 p-8">
                <div className="space-y-2 text-center">
                  ...
                  <p className="text-muted-foreground text-xs">
                    Run a partition to see results here.
                  </p>
                </div>
              </div>
            ) : (
                    ...
                    >
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span className="inline-block w-fit max-w-full truncate align-middle font-medium">
                                  {partition.file?.filename || "Unknown"}
                                </span>
                              </TooltipTrigger>
                              <TooltipContent>
                                {partition.file?.filename || "Unknown"}
                      ...
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                        <span className="font-mono text-xs">
                          {partition.model || "Unknown"}
                        </span>
                      </TableCell>
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span className="inline-block w-fit max-w-full truncate align-middle font-mono">
                              {partition.key || "—"}
                            </span>
                          </TooltipTrigger>
                          <TooltipContent>
                      ...
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                        {partition.created_at
                          ? new Date(partition.created_at).toLocaleString()
                          : "—"}
                      </TableCell>
                      <TableCell className="px-3 text-right">
                        {canDeletePartition && (
                          <Popover
                            open={openMenuId === partition.id}
                            onOpenChange={(open) =>
                              setOpenMenuId(open ? partition.id : null)
                            }
                          >
                            <PopoverTrigger asChild>
                              <Button
                                ...
                                disabled={deletingId === partition.id}
                              >
                                {deletingId === partition.id ? (
                            ...
                            >
                              <button
                                ...
                                onClick={(e) =>
                                  handleDeleteClick(e, partition.id)
                                }
                              >
          ...
          <div className="flex items-center justify-between p-4">
            <div className="text-muted-foreground text-xs">
              {partitions.length > 0 &&
                `Showing ${partitions.length} partition${partitions.length !== 1 ? "s" : ""}`}
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                ...
                onClick={partitionsPrevPage}
                disabled={
                  !partitionsMetadata?.before || partitionsQuery.isLoading
                }
              >
              ...
              <Button
                ...
                size="sm"
                className="group px-2 text-xs"
                onClick={partitionsNextPage}
                disabled={
                  !partitionsMetadata?.after || partitionsQuery.isLoading
                }
              >
                Next
        ...
        <AlertDialogContent className="gap-0 rounded-2xl border-none bg-gray-50 p-0 shadow-2xl">
          <div className="rounded-2xl bg-white shadow-xs">
            <AlertDialogHeader className="p-6 pb-4">
              ...
              <AlertDialogDescription>
                Are you sure you want to delete this partition result? This
...
// Mount-guard that triggers a refetch each time the dialog opens (mounted) — replaces the
// prior useEffect that watched for open transitions.
function RefetchOnOpen({ refetch }: { refetch: () => unknown }) {
  useMountEffect(() => {
    void refetch();
  });
  return null;
