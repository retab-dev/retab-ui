...
export const SplitHistoryDialog = ({
  ...
  // Pagination state
  const [splitsBefore, setSplitsBefore] = useState<string | null>(null);
  const [splitsAfter, setSplitsAfter] = useState<string | null>(null);
  ...
  // Delete state. UI signaling: hide the delete affordance when the split
  // primitive capability is absent (no distinct split-delete policy exists, so
  // the page primitive governs history delete).
  const canDeleteSplit = useCanOrganization("rbac:primitive:split");
  const { mutateAsync: deleteSplit } = useDeleteSplit();
  ...
  const handleSearchChange = (value: string) => {
    ...
    setSplitsBefore(null);
    setSplitsAfter(null);
  };

  // Query splits
  const splitsQuery = useSplitList(
    {
      before: splitsBefore || undefined,
      after: splitsAfter || undefined,
      limit: limit,
  ...
  const splitsMetadata = splitsQuery.data?.list_metadata;
  ...
  // Pagination handlers
  const splitsNextPage = useCallback(() => {
    if (splitsMetadata?.after) {
      setSplitsAfter(splitsMetadata.after);
      setSplitsBefore(null);
    }
  }, [splitsMetadata]);

  const splitsPrevPage = useCallback(() => {
    if (splitsMetadata?.before) {
      setSplitsBefore(splitsMetadata.before);
      setSplitsAfter(null);
    }
  }, [splitsMetadata]);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteSplit(deleteConfirmId);
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="flex max-h-[80vh] min-h-[80vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl">
          <DialogHeader className="px-6 pt-6 pb-4">
            <DialogTitle>Split History</DialogTitle>
            <DialogDescription className="sr-only">
              View and load your previous split results
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-4 p-8">
                <div className="space-y-2 text-center">
                  ...
                  <p className="text-muted-foreground text-xs">
                    Run a split to see results here.
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  {splits.map((split) => (
                    ...
                    >
                      ...
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                        {
                          new Set(
                            (split.output ?? []).flatMap((s) => s.pages ?? []),
                          ).size
                        }
                      </TableCell>
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                      ...
                      <TableCell className="px-3 text-right">
                        {canDeleteSplit && (
                          <Popover
                            ...
                            onOpenChange={(open) =>
                              setOpenMenuId(open ? split.id : null)
                            }
                          >
                            <PopoverTrigger asChild>
                              <Button
          ...
          <div className="flex items-center justify-between p-4">
            <div className="text-muted-foreground text-xs">
              {splits.length > 0 &&
                `Showing ${splits.length} split${splits.length !== 1 ? "s" : ""}`}
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                ...
                onClick={splitsPrevPage}
                disabled={!splitsMetadata?.before || splitsQuery.isLoading}
              >
              ...
              <Button
                ...
                size="sm"
                className="group px-2 text-xs"
                onClick={splitsNextPage}
                disabled={!splitsMetadata?.after || splitsQuery.isLoading}
              >
                Next
        ...
        <AlertDialogContent className="gap-0 rounded-2xl border-none bg-gray-50 p-0 shadow-2xl">
          <div className="rounded-2xl bg-white shadow-xs">
            <AlertDialogHeader className="p-6 pb-4">
              ...
              <AlertDialogDescription>
                Are you sure you want to delete this split result? This action
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
          </div>
