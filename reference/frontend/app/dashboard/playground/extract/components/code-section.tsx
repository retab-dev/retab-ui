...
import {
  generatePythonRetabCode,
  generateTypeScriptRetabCode,
  generateGoCode,
  generatePhpCode,
  generateDotnetCode,
  generateRubyCode,
...
const CodeSection = ({
  ...
  const handleCreateKey = async () => {
    // Fail-closed guard: never call the mutation without the capability, even
    // if a stale/forged event reaches this handler.
    if (!canCreateApiKey) {
      return;
    }
  ...
  const generatePythonRetabCodeWithActualVariables = () => {
    return generatePythonRetabCode(
      currentSchema,
      configValues,
      getDisplayApiKey(),
    );
  ...
  const generateTypeScriptRetabCodeWithActualVariables = () => {
    return generateTypeScriptRetabCode(
      currentSchema,
      configValues,
      getDisplayApiKey(),
    );
  ...
  const generatedSdkSnippets = [
    {
      ...
      icon: "go",
      language: "go",
      code: generateGoCode(currentSchema, configValues, getDisplayApiKey()),
    },
    {
      value: "php",
      label: "PHP",
      icon: "php",
      language: "php",
      code: generatePhpCode(currentSchema, configValues, getDisplayApiKey()),
    },
    {
      value: "dotnet",
      label: ".NET",
      icon: "dotnet",
      language: "csharp",
      code: generateDotnetCode(currentSchema, configValues, getDisplayApiKey()),
    },
    {
      value: "ruby",
      label: "Ruby",
      icon: "ruby",
      language: "ruby",
      code: generateRubyCode(currentSchema, configValues, getDisplayApiKey()),
    },
    {
      value: "rust",
      label: "Rust",
      icon: "rust",
      language: "rust",
      code: generateRustCode(currentSchema, configValues, getDisplayApiKey()),
    },
    {
      value: "java",
      label: "Java",
      icon: "java",
      language: "java",
      code: generateJavaCode(currentSchema, configValues, getDisplayApiKey()),
    },
  ] as const;

  const generateCurlCodeWithActualVariables = () => {
    return generateCurlCode(currentSchema, configValues, getDisplayApiKey());
  };
