...
import {
  useParseList,
  useDeleteParse,
} from "@/app/dashboard/widgets/queries/parses";
...
export const ParseHistoryDialog = ({
  open,
  onOpenChange,
  onSelectParse,
}: {
  ...
  onSelectParse?: (parsing: StoredParse) => void;
}) => {
  // Pagination state
  const [parsingsBefore, setParsesBefore] = useState<string | null>(null);
  const [parsingsAfter, setParsesAfter] = useState<string | null>(null);
  ...
  // Delete state. UI signaling: hide the delete affordance when the parse
  // primitive capability is absent (no distinct parse-delete policy exists, so
  // the page primitive governs history delete).
  const canDeleteParse = useCanOrganization("rbac:primitive:parse");
  const { mutateAsync: deleteParse } = useDeleteParse();
  ...
  const handleSearchChange = (value: string) => {
    ...
    setParsesBefore(null);
    setParsesAfter(null);
  };

  // Query parsings
  const parsingsQuery = useParseList(
    {
      before: parsingsBefore || undefined,
      after: parsingsAfter || undefined,
      limit: limit,
  ...
  const parsings = parsingsQuery.data?.data || [];
  const parsingsMetadata = parsingsQuery.data?.list_metadata;
  ...
  // Pagination handlers
  const parsingsNextPage = useCallback(() => {
    if (parsingsMetadata?.after) {
      setParsesAfter(parsingsMetadata.after);
      setParsesBefore(null);
    }
  }, [parsingsMetadata]);

  const parsingsPrevPage = useCallback(() => {
    if (parsingsMetadata?.before) {
      setParsesBefore(parsingsMetadata.before);
      setParsesAfter(null);
    }
  }, [parsingsMetadata]);

  // Handle parsing click
  const handleParseClick = useCallback(
    (parsing: StoredParse) => {
      if (onSelectParse) {
        onSelectParse(parsing);
      }
      onOpenChange(false);
    },
    [onSelectParse, onOpenChange],
  );

  // Delete handlers
  const handleDeleteClick = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (!canDeleteParse) return;
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteParse(deleteConfirmId);
        ...
        <DialogContent className="flex max-h-[80vh] min-h-[80vh] flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl">
          <DialogHeader className="px-6 pt-6 pb-4">
            ...
            <DialogDescription className="sr-only">
              View and load your previous parsing results
            </DialogDescription>
          </DialogHeader>
                ...
                <TableBody>
                  {parsings.map((parsing) => (
                    <TableRow
                      ...
                      onClick={() => handleParseClick(parsing)}
                    >
                      ...
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                        <span className="font-mono text-xs">
                          {parsing.model || "Unknown"}
                        </span>
                      </TableCell>
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                        {parsing.output?.pages?.length || 0}
                      </TableCell>
                      <TableCell className="text-muted-foreground px-3 text-xs whitespace-nowrap">
                        {new Date(parsing.created_at ?? "").toLocaleString()}
                      </TableCell>
                      <TableCell className="px-3 text-right">
                        {canDeleteParse && (
                          <Popover
                            open={openMenuId === parsing.id}
                            onOpenChange={(open) =>
                              setOpenMenuId(open ? parsing.id : null)
                            }
                          >
                            <PopoverTrigger asChild>
                              <Button
                                ...
                                disabled={deletingId === parsing.id}
                              >
                                {deletingId === parsing.id ? (
                            ...
                            >
                              <button
                                ...
                                onClick={(e) =>
                                  handleDeleteClick(e, parsing.id)
                                }
                              >
                                <Trash2 className="h-3.5 w-3.5" />
                                <span>Delete parsing</span>
                              </button>
                            </PopoverContent>
                          </Popover>
                        )}
          ...
          <div className="flex items-center justify-between p-4">
            <div className="text-muted-foreground text-xs">
              {parsings.length > 0 &&
                `Showing ${parsings.length} parsing${parsings.length !== 1 ? "s" : ""}`}
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                ...
                onClick={parsingsPrevPage}
                disabled={!parsingsMetadata?.before || parsingsQuery.isLoading}
              >
              ...
              <Button
                ...
                size="sm"
                className="group px-2 text-xs"
                onClick={parsingsNextPage}
                disabled={!parsingsMetadata?.after || parsingsQuery.isLoading}
              >
                Next
        ...
        <AlertDialogContent className="gap-0 rounded-2xl border-none bg-gray-50 p-0 shadow-2xl">
          <div className="rounded-2xl bg-white shadow-xs">
            <AlertDialogHeader className="p-6 pb-4">
              ...
              <AlertDialogDescription>
                Are you sure you want to delete this parsing result? This action
                cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
          </div>
