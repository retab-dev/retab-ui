...
export function JsonModeEditor({
  ...
  const schemaText = React.useMemo(() => JSON.stringify(schema, null, 2), [schema])
  ...
  React.useEffect(() => {
    setJsonState((current) =>
      current.status === "synced" ? { status: "synced", text: schemaText } : current
    )
  }, [schemaText])
  ...
  return (
    <div className="flex min-h-[420px] flex-col gap-3">
      <textarea
        spellCheck={false}
        readOnly={readOnly}
        className="min-h-0 flex-1 resize-none overflow-auto rounded-lg border bg-muted/30 p-3 font-mono text-xs outline-none focus-visible:ring-2 focus-visible:ring-ring"
