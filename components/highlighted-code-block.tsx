...
export function HighlightedCodeBlock({
  ...
  return (
    <div
      ...
      className={cn(
        "bg-code text-code-foreground relative m-0! overflow-hidden rounded-lg border",
        className,
      )}
    >
      ...
      {shouldShowFallback ? (
        <pre
          aria-hidden="true"
          className={cn(
            "text-code-foreground pointer-events-none absolute inset-0 z-[1] m-0 overflow-hidden px-4 py-3.5 font-mono text-[0.8rem] leading-[1.625] whitespace-pre",
            maxHeightClassName,
          )}
        >
      ...
      {isVisible ? (
        ...
        <div className={cn("bg-code min-h-72", maxHeightClassName)} />
      )}
    </div>
  );
}
