...
export function LogoStrip({ logos }: { logos: readonly LogoContent[] }) {
  ...
  return (
    <div className="w-full overflow-hidden pb-8 min-[961px]:pb-[70px]">
      ...
      <div
        aria-hidden="true"
        className="flex w-max min-w-0 items-center gap-12 px-6 text-neutral-950 motion-safe:animate-[homepage-logo-marquee_28s_linear_infinite] motion-reduce:hidden min-[1120px]:hidden"
      >
      ...
      <div
        aria-hidden="true"
        className="hidden flex-wrap items-center justify-center gap-x-10 gap-y-6 px-6 text-neutral-950 motion-reduce:flex min-[1120px]:hidden"
      >
        {logos.map((logo) => (
          <BrandLogo key={logo.id} logo={logo} />
        ))}
      </div>
      <div className="hidden w-full min-w-0 items-center justify-between gap-8 text-neutral-950 min-[1120px]:flex">
        {logos.map((logo) => (
          <BrandLogo key={logo.id} logo={logo} />
        ))}
      </div>
