...
export function MobileNavigation({ content }: { content: HeaderContent }) {
  const firstGroupId =
    content.navItems.find((item) => item.kind === "group")?.group.id ?? null;
  const [isOpen, setIsOpen] = useState(false);
  const [openGroupId, setOpenGroupId] = useState<string | null>(firstGroupId);
              ...
              <div key={group.id}>
                ...
                >
                  {group.sections.map((section) => {
                    const headingId = getMobileSectionId(
                      group.id,
                      section.title,
                    );
                        ...
                        <div className="grid gap-3">
                          {section.items.map((item) => (
                            <Link
                              ...
                              onClick={closeMobileMenu}
                              className={cn(
                                "text-foreground hover:bg-accent hover:text-accent-foreground focus-visible:bg-accent focus-visible:text-accent-foreground -mx-2 rounded-md px-2 py-1.5 text-xl font-normal transition-colors motion-reduce:transition-none",
                                focusRing,
                              )}
                            >
