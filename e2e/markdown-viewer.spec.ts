...
const MARKDOWN_VIEW_PATH = "/view/markdown-viewer";
...
test("pretext markdown docs demo renders as one continuous virtual document", async ({
  ...
  await expect(viewer.locator("[data-markdown-page]")).toHaveCount(0);
  await expect(
    viewer.locator('[data-slot="markdown-empty-state"]'),
  ).toHaveCount(0);
  ...
  await expect(page).toHaveURL(new RegExp(`${MARKDOWN_VIEW_PATH}$`));
  ...
  await page.goto(`${MARKDOWN_VIEW_PATH}#raw-html-policy`);
  await expect(
...
test("pretext markdown docs demo resolves initial hash fragments inside the virtual viewport", async ({
  ...
  await page.goto(`${MARKDOWN_VIEW_PATH}#release-readiness-matrix`);
...
test("pretext markdown docs demo searches offscreen rendered and source content", async ({
  ...
  const searchStatus = viewer.locator('[data-slot="markdown-search-status"]');
...
test("pretext markdown docs demo keeps scroll stable as rich blocks settle", async ({
  ...
  await page.goto(`${MARKDOWN_VIEW_PATH}#diagrams`);
  ...
  await expect
    .poll(() => settledMermaidDiagramCount(viewer))
    .toBeGreaterThan(1);
  await page.waitForTimeout(300);
  await expect
...
test("pretext markdown docs demo stays continuous on a narrow mobile viewport", async ({
  ...
  await expect(viewer).toBeVisible();
  await expect(
    viewer.getByRole("heading", { name: "Markdown Viewer" }),
  ).toBeVisible();
  await expect(viewer.getByText("Page 1 of")).toHaveCount(0);
  await expect(viewer.locator("[data-markdown-page]")).toHaveCount(0);
  await expect.poll(() => mountedChunkCount(viewer)).toBeLessThan(24);
...
async function expectRawHtmlLinkPolicy(viewer: Locator) {
  ...
  await expect(external).toHaveAttribute("target", "_blank");
  await expect(external).toHaveAttribute("rel", "noopener noreferrer");
  await expect(internal).toHaveAttribute(
    "href",
...
async function mountedChunkCount(viewer: Locator) {
  return viewer.locator("[data-markdown-chunk]").count();
}
