...
| `anchorLink` | `FieldAnchorLink`       | Opt into field-level anchor linking — every scalar becomes a hoverable card that reports its path and highlights when active (wire it from `useAnchoredFieldLink`). |
