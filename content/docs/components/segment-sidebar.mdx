...
`ViewerSidebar` owns viewer placement, width, collapse state, and the accessible
label for the rail. `SegmentSidebar` owns only the segment-row model and
interaction semantics. Nesting `SegmentSidebar` inside `ViewerSidebar` is the
intended composition when segments are the viewer's rail content.
