...
<MermaidDiagram
  chart={`
flowchart LR
  field["Field component<br/>json-form or custom list"]
  path["Field path<br/>owner.name"]
  link["useSourceLink<br/>hover + pinned state"]
  map[("SourceMap<br/>path to source")]
  adapter["PDF source adapter<br/>scroll target + overlay"]
  viewer["Viewer<br/>highlight + scroll"]

field -->|"hover or select"| path
path --> link
map -->|"lookup active path"| link
link -->|"active source"| adapter
adapter --> viewer
...
class field,path input
class link,map state
class adapter,viewer output
`}
/>
...
export function Example({ response, form }) {
  ...
  return (
    <div className="flex h-full">
      ...
      />
      <JsonForm form={form} schema={response.schema} sourceLink={link} />
    </div>
  )
}
```
...
`SourceTarget` (a `scrollTo`) and an overlay renderer.
...
| `clear()`                    | clear hover and pin.                                      |
| `activeSource`               | the active field's source — feed to the overlay.          |
| `activePath`                 | the effective active path (hover wins over pin).          |
| `hoverPath` / `pinnedPath`   | the transient hovered and persisted pinned paths.         |
...
| `renderPdfSourceOverlay(source)` | a `renderPageOverlay` drawing the highlight on its page — pass `activeSource`. |
| `pdfAnchorToTarget(anchor)`      | normalized bbox -> page `%` target.                                            |
