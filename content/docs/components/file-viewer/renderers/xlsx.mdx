...
- **Row + column virtualization.** Built on Retab's local fixed-grid
  virtualizer: only the visible window of cells is rendered, with tight overscan
  and pooled rows to avoid blank flashes while scrolling.
