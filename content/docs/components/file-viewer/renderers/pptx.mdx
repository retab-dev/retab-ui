...
<ComponentPreview name="pptx-viewer-demo" contentClassName="p-0" />
...
- **Canvas rendering, client-side.** Slides parse and rasterize in the browser —
  no conversion service. The slide size is read from the loaded `pptxviewjs`
  presentation, so every slide reserves its box before anything is drawn without
...
- **Adaptive render scale.** The viewer caps high-DPI raster work while preserving
  zoom, avoiding runaway canvas costs on dense displays.
...
- **Shared thumbnail source.** File thumbnails reuse the same parsed deck source
  and bitmap cache as the full viewer instead of parsing the presentation a
  second time.
...
convert to PDF server-side and use the [PDF Viewer](/docs/components/file-viewer/renderers/pdf).
...
<ComponentSource src="registry/new-york-v4/ui/pptx-viewer.tsx" />
