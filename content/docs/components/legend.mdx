...
Hovering, keyboard focus, persistent selection, and `currentPage` are separate
states. Wire the legend to the same `useSegmentInteraction` object as a
...
a floating card, and a vertical rail). One shared interaction state dims the
