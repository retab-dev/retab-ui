...
export function FileIntakeTarget() {
...
Use `getButtonProps` when the trigger is a real button. Use `getTriggerProps`
for custom trigger elements, validation-only targets, disabled states, and
parent-owned queues.
...
```tsx
export function ToolbarAttachment() {
  const dropzone = useDropzone()

  return (
    <div {...dropzone.getRootProps()}>
      <input {...dropzone.getInputProps()} />
      <button {...dropzone.getButtonProps()}>Add files</button>
    </div>
  )
}
```
...
import { useDropzone, type DropzoneIntake } from "@/components/ui/dropzone"

export function PdfValidationTarget() {
  const [lastIntake, setLastIntake] = React.useState<DropzoneIntake>({
    acceptedFiles: [],
    fileRejections: [],
  })
  const dropzone = useDropzone({
  ...
  return (
    <div {...dropzone.getRootProps(dropzone.getTriggerProps())}>
      <input {...dropzone.getInputProps()} />
      Accepted: {lastIntake.acceptedFiles.length}
      Rejected: {lastIntake.fileRejections.length}
    </div>
  )
}
```
