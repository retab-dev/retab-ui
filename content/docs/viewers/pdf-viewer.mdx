...
`thumbnailWidth` controls the thumbnail image width. `ViewerSidebar width`
controls the sidebar shell.

## Usage

`PdfViewer` is the easy API. It is the preassembled PDF viewer with
`PdfViewerHeader`, `ViewerBody`, `ViewerSurface`, and
`PdfViewerPages`. Compose `PdfViewerThumbnails` inside
`ViewerSidebar` when a thumbnail rail is needed.
...
<PdfViewerProvider source={{ kind: "url", url, fileName: "document.pdf" }}>
  <ViewerRoot>
    <WorkflowHeader />
    <ViewerBody>
      <ViewerSidebar aria-label="Workflow pages">
        <PageRibbon />
      </ViewerSidebar>
      <ViewerSurface>
