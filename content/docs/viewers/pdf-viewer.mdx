...
edge plus a floating overlay. Pass them through `slots`.
...
`scrollToPageTarget(pageNumber, target, options)` scrolls a page's normalized
vertical position (`target.top`, a percentage `[0, 100]`) into view, and
`getViewportElement()` returns the scrolling viewport. Overlay boxes still use
full `{ left, top, width, height }` geometry; imperative scrolling only needs the
vertical target.
