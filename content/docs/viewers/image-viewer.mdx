...
normalized boxes map with simple percentages. The props include both
`frameNumber` and `pageNumber`; they are the same value in `ImageViewer`.
`pageNumber` is kept for compatibility with the PDF viewer API.
...
The API mirrors the [PDF Viewer](/docs/viewers/pdf-viewer). `frameNumber` is the
native image-viewer term. `pageNumber` is the same 1-based frame index and is
kept for PDF-viewer compatibility (a TIFF page; always `1` for a single image).
...
| `renderPageOverlay`      | `(props) => ReactNode`       | Per-frame overlay; receives `{ frameNumber, pageNumber, width, height, scale, rotation }`. |
