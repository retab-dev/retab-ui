...
export function resolveFileSystemListContinuityExpansion<TRuntimeInput>({
  ...
  switch (transition.kind) {
    case "query-enter":
    case "query-update":
      return collectFileSystemListDirectoryPaths(transition.next.input.itemPaths)
    case "decoration":
    case "input":
    case "path":
    case "query-exit":
  ...
  switch (event.type) {
    ...
    case "domain.folderLoadSucceeded":
      return state.pendingRevealPath?.startsWith(event.path)
        ? result({ ...state, modelRevision: `reveal:${state.pendingRevealPath}` })
        : result(state)
...
function captureFileSystemListContinuitySnapshot<TRuntimeInput>({
  ...
  if (state.phase !== "capturing" || !transition || transition.previous !== identity) {
    return result(state)
  }
...
function applyFileSystemListContinuityModel<TRuntimeInput>({
  ...
  const nextState = {
    ...
    phase: state.pendingRevealPath ? ("revealing" as const) : ("stable" as const),
    transition: null,
  }
