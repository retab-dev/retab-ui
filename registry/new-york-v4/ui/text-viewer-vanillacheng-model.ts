...
/*
 * VanillaCheng is a product-shaped port of Chenglou's Pretext markdown-chat
 * demo, adapted under the MIT License from:
 * https://github.com/chenglou/pretext/blob/main/pages/demos/markdown-chat.ts
 * https://github.com/chenglou/pretext/blob/main/pages/demos/markdown-chat.model.ts
 *
...
export type PreparedVanillaChengTemplate = {
...
export type VanillaChengMessageInstance = {
  bottom: number
  prepared: PreparedVanillaChengTemplate
  frame: TemplateFrame
  top: number
}

export type VanillaChengFrame = {
  chatWidth: number
  messages: VanillaChengMessageInstance[]
  totalHeight: number
}
...
export function createPreparedVanillaChengTemplates(
  markdown: string
): PreparedVanillaChengTemplate[] {
...
export function getMaxChatWidth(viewportWidth: number): number {
  return Math.max(
    240,
    Math.min(MAX_CHAT_WIDTH, viewportWidth - PAGE_MARGIN * 2)
  )
}

export function buildVanillaChengFrame(
  templates: readonly PreparedVanillaChengTemplate[],
  chatWidth: number
): VanillaChengFrame {
  ...
  const messages: VanillaChengMessageInstance[] = new Array(templates.length)
  ...
  const totalHeight = messages.length === 0 ? 24 : y - MESSAGE_GAP + 10
...
export function findVisibleRange(
  ...
  if (frame.messages.length === 0) return { end: 0, start: 0 }

  const minY = Math.max(0, scrollTop)
  const maxY = Math.max(minY, scrollTop + viewportHeight)
  let low = 0
...
function parseBlockTokens(
  ...
  for (let index = 0; index < tokens.length; index++) {
    ...
    switch (token.type) {
      ...
      case "html": {
        const htmlToken = token as Token & {
          block?: boolean
          pre?: boolean
          text: string
        }
        const htmlText =
          htmlToken.text.trim().length > 0 ? htmlToken.text : token.raw
        if (htmlToken.block || htmlToken.pre) {
          appendBlockGroup(
            blocks,
      ...
      case "text": {
        ...
        if (Array.isArray(textToken.tokens) && textToken.tokens.length > 0) {
          appendBlockGroup(
            blocks,
            buildInlineBlocks(textToken.tokens, "body", ctx),
            BLOCK_GAP
          )
        } else {
          appendBlockGroup(
            blocks,
            buildPlainTextBlocks(textToken.text, "body", ctx),
            BLOCK_GAP
          )
        }
        continue
    ...
    for (let index = 0; index < tokenList.length; index++) {
      ...
      switch (token.type) {
        case "text": {
          ...
          if (Array.isArray(textToken.tokens) && textToken.tokens.length > 0) {
            walk(textToken.tokens, marks)
          } else {
            pushPiece(createTextPiece(textToken.text, marks, variant))
          }
          continue
        }
        ...
        case "image":
          pushPiece(
            createImagePiece(token.text.length > 0 ? token.text : token.href)
          )
          continue
        ...
        case "checkbox": {
          const checkboxToken = token as Token & { checked?: boolean }
          pushPiece(
            createTextPiece(
              checkboxToken.checked ? "[x] " : "[ ] ",
              marks,
              variant
            )
          )
...
function createCodePiece(text: string): InlinePiece | null {
  ...
  return {
    breakMode: "normal",
    className: "vc-frag vc-frag--code",
...
function createImagePiece(text: string): InlinePiece {
  return {
    breakMode: "never",
    className: "vc-frag vc-frag--chip",
...
function resolveTextClassName(
  ...
  let className = "vc-frag"

  switch (variant) {
    case "heading-1":
      className += " vc-frag--heading-1"
      break
    case "heading-2":
      className += " vc-frag--heading-2"
      break
    case "body":
      className += " vc-frag--body"
      break
  }
...
function resolveListMarkerClassName(
  ...
  if (item.task) return "vc-block-marker vc-block-marker--task"
  return list.ordered
    ? "vc-block-marker vc-block-marker--ordered"
    : "vc-block-marker vc-block-marker--bullet"
}
...
function layoutTemplateFrame(
  template: PreparedVanillaChengTemplate,
