...
const GITHUB_ALERT_LABELS = new Map([
  ["caution", "Caution"],
  ["danger", "Danger"],
  ["important", "Important"],
  ["info", "Info"],
...
function parseBlockTokens(
  ...
  for (const token of tokens) {
    ...
    switch (token.type) {
      ...
      case "blockquote":
        {
          const alertTokens = normalizeGithubAlertBlockquoteTokens(token.tokens)
          appendBlockGroup(
            blocks,
            parseBlockTokens(alertTokens ?? token.tokens ?? [], {
              ctx: {
...
function normalizeGithubAlertBlockquoteTokens(tokens: readonly Token[] | undefined) {
  if (!tokens?.length) return null
  const firstBlock = tokens[0]
  if (firstBlock?.type !== "paragraph" || !Array.isArray(firstBlock.tokens)) {
    return null
  }
  ...
  const match = firstInline.text.match(/^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION|INFO|DANGER)\]\s*(?:\r?\n)?/i)
  if (!match) return null

  const kind = match[1]!.toLowerCase()
  const label = GITHUB_ALERT_LABELS.get(kind) ?? "Note"
  return [
    {
      ...firstBlock,
      raw: firstBlock.raw.replace(match[0], `${label}: `),
      text: firstBlock.text.replace(match[0], `${label}: `),
      tokens: [
        {
          ...firstInline,
          raw: firstInline.raw.replace(match[0], `${label}: `),
          text: firstInline.text.replace(match[0], `${label}: `),
        },
        ...firstBlock.tokens.slice(1),
      ],
    },
    ...tokens.slice(1),
  ] as Token[]
...
function createTextPiece(
  ...
  return {
    ...
    href: marks.href,
    text: transformMarkdownProseText(text),
    title: marks.title,
  }
}

function transformMarkdownProseText(text: string) {
  return replaceEmojiShortcodes(transformTypography(text))
}

function transformTypography(text: string) {
  return text
    .replace(/<->/g, "↔")
    .replace(/->/g, "→")
    .replace(/<-/g, "←")
...
function replaceEmojiShortcodes(text: string) {
  return text.replace(/:([a-z0-9_+-]+):/gi, (match, shortcode: string) => {
    return EMOJI_SHORTCODES.get(shortcode.toLowerCase()) ?? match
  })
}
