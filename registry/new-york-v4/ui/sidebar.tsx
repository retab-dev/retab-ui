...
// When a sidebar control renders a native `<button>` (either as its default
// element or as the `asChild` child), give it an explicit `type="button"` so it
// never submits an enclosing form. When the consumer renders a different element
// (e.g. an `<a>`) or sets an explicit `type`, we leave the type alone.
function getSidebarButtonType({
  ...
  if (!asChild) {
    return defaultElement === "button" ? "button" : undefined
  }

  if (
    React.isValidElement<React.ButtonHTMLAttributes<HTMLButtonElement>>(child) &&
    child.type === "button" &&
    child.props.type == null
  ) {
    return "button"
  }
...
function SidebarTrigger({
  ...
  // When `asChild`, the consumer owns the rendered element, so we slot onto a
  // single child element rather than injecting the icon/label (which would feed
  // Slot multiple children and crash).
  if (asChild) {
    return (
      <Button
        asChild
        ...
        onClick={handleClick}
        type={getSidebarButtonType({ asChild, child: children })}
        {...props}
      >
        {children}
      </Button>
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-size={size}
      data-active={isActive}
      type={getSidebarButtonType({ asChild, defaultElement: "a", child: children })}
      className={cn(
