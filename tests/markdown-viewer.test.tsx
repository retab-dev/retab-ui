...
function mockObjectUrls(url = "blob:markdown-download") {
...
describe("MarkdownViewer", () => {
  it("hydrates inline server markup into rendered Markdown content", async () => {
    ...
    const serverHtml = renderToString(
      <MarkdownViewer source={source} controls={false} />,
    );
    expect(serverHtml).toContain('data-slot="markdown-virtual-canvas"');
    ...
    try {
      ...
      const chunkObservers = resizeObservers.filter(
        ({ target }) =>
          ...
          target.hasAttribute("data-markdown-chunk"),
      );
  ...
  it("shows and resets generated Markdown download failures", async () => {
    ...
    expect(
      document.querySelector('[data-slot="markdown-download-error"]'),
    ).toBeTruthy();
  ...
  it("downloads local Blob sources through a revoked object URL", async () => {
    ...
    expect(clicks).toEqual([
      { href: "blob:local-markdown", download: "local-blob.md" },
    ]);
  ...
  it("renders GFM inline semantics for breaks, strike, and autolinks", async () => {
    ...
    expect(strikethrough?.className).toContain(
      "decoration-muted-foreground/70",
    );
  ...
  it("renders GFM task list checkboxes as read-only controls", async () => {
    ...
    expect(completed.getAttribute("data-pretext-task-checkbox")).toBe(
      "checked",
    );
    ...
    expect(pending.getAttribute("data-pretext-task-checkbox")).toBe(
      "unchecked",
    );
  ...
  it("describes Mermaid diagrams with source-derived accessible summaries", async () => {
    ...
    expect(pieDiagram?.getAttribute("data-diagram-reserved-height")).toBe(
      "224",
    );
  });
  ...
  it("keeps diagram loading and ready states inside stable reserved height", async () => {
    ...
    expect(pendingBody?.getAttribute("aria-label")).toBe(
  ...
  it("renders diff code fences with added and removed line styling", async () => {
    ...
    const { container } = render(
      <MarkdownViewer source={markdownSource(source)} controls={false} />,
    );
  ...
  it("renders table cell inline Markdown and copies rendered cells as TSV", async () => {
    ...
    expect(headers[0]?.id).toMatch(/^markdown-table-\d+-column-1$/);
    ...
    expect(headers[1]?.id).toMatch(/^markdown-table-\d+-column-2$/);
  ...
  it("renders safe inline raw HTML elements with styled components", async () => {
    ...
    expect(
      container.querySelector("small")?.getAttribute("onclick"),
    ).toBeNull();
  ...
  it("keeps TOML frontmatter as a first-class chunk", async () => {
    ...
    const frontmatter = container.querySelector(
      '[data-markdown-frontmatter="toml"]',
    );
    expect(frontmatter?.textContent).toContain('title = "Release Notes"');
