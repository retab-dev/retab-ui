...
describe("description operations", () => {
  it("stripDescriptions removes descriptions from schema-bearing rest keywords", () => {
    ...
    expect((pair.additionalItems as JSONSchema7).description).toBeUndefined();
    expect(
  ...
  it("stripDescriptions preserves schema map keys that collide with object prototype keys", () => {
    ...
    expect(
      Object.prototype.hasOwnProperty.call(out.patternProperties, "__proto__"),
    ).toBe(true);
    ...
    expect(
      (out.patternProperties!.__proto__ as JSONSchema7).description,
    ).toBeUndefined();
    expect(
      (
    ...
    expect(
      (out.patternProperties!.constructor as JSONSchema7).description,
    ).toBeUndefined();
  });
});
...
describe("definition operations", () => {
  ...
  it("resolves percent-encoded definition refs and normalizes them on rename", () => {
    ...
    const defId = d.defs.find(
      (definition) => definition.name === "Line Item",
    )!.id;
  ...
  it("resolves lowercase percent-encoded unicode definition refs", () => {
    ...
    const defId = d.defs.find(
      (definition) => definition.name === "Café Item",
    )!.id;
  ...
  it("resolves refs that percent-encode slash and tilde in definition names", () => {
    ...
    const out = json(d);
    expect((out.properties!.item as JSONSchema7).$ref).toBe("#/$defs/D~1E~0F");
  });
  ...
  it("prefers JSON Pointer semantics over tolerant encoded-name aliases", () => {
    ...
    const tildeOneId = d.defs.find(
      (definition) => definition.name === "A~1B",
    )!.id;
  ...
  it("renameDefinition updates refs inside schema-bearing rest keywords", () => {
    ...
    const moneyId = d.defs.find(
      (definition) => definition.name === "Money",
    )!.id;
    ...
    expect(
      (
        (out.properties as Record<string, JSONSchema7>).tuple
    ...
    expect((tuple.unevaluatedItems as JSONSchema7).$ref).toBe("#/$defs/Amount");
    expect(
  ...
  it("renameDefinition does not rewrite refs into the unmodeled definitions namespace", () => {
    ...
    expect(properties.legacy.$ref).toBe("#/definitions/Money");
    expect((out.additionalProperties as JSONSchema7).$ref).toBe(
      "#/definitions/Money",
    );
  });
