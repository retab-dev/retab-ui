...
describe("json table and DataCell architecture", () => {
  ...
  it("keeps primitive active-cell store as the only flushSync boundary", () => {
    ...
    const replacementContent = readFileSync(
      join(repoRoot, allowedFile),
      "utf8",
    );
    expect(
  ...
  it("keeps primitive interaction tests on the shared row harness", () => {
    const interactionUtilityFile =
      "tests/json-table-interaction-test-utils.tsx";
  ...
  it("keeps primitive and structured commit controllers separated", () => {
    ...
    expect(primitiveCommitControllerContent.includes("onCellCommit")).toBe(
      true,
    );
    expect(
  ...
  it("keeps primitive echo recognition narrow and non-serializing", () => {
    const editStoreFile =
      "components/json-table/json-table-primitive-edit-store.ts";
    const editStoreContent = readFileSync(
      join(repoRoot, editStoreFile),
      "utf8",
    );
  ...
  it("keeps primitive controls on exact kind-specific props", () => {
    ...
    const editModelContent = readFileSync(
      join(repoRoot, editModelFile),
      "utf8",
    );
    ...
    expect(contractContent.includes("DataCellTextInputControlProps")).toBe(
      true,
    );
    ...
    expect(registryContent.includes("useDataCellPrimitiveSession<")).toBe(
      false,
    );
    ...
    expect(editModelContent.includes("isDataCellBooleanCommitValue")).toBe(
      true,
    );
    ...
    expect(inputControlContent.includes("useDataCellOpeningContext")).toBe(
      true,
    );
    ...
    expect(inputControlContent.includes("DataCellTextControlProps")).toBe(
      false,
    );
    ...
    expect(pickerControlContent.includes("previousValue: value as")).toBe(
      false,
    );
    ...
    expect(editModelContent.includes("DataCellNumberCommitHandler")).toBe(
      false,
    );
    expect(editModelContent.includes("DataCellPickerCommitHandler")).toBe(
      false,
    );
    ...
    expect(actionsContent.includes("createDefaultPointerEditAction")).toBe(
      true,
    );
    ...
    expect(registryContent.includes("dataCellKeyActionWithAdapter")).toBe(
      false,
    );
    ...
    expect(registryContent.includes("integer: DataCellInputControl")).toBe(
      true,
    );
  ...
  it("keeps json-table DataCell adaptation pure and outside the renderer", () => {
    ...
    const displayValueFile =
      "components/json-table/json-table-display-value.ts";
    ...
    expect(primitiveContent.includes("JsonTablePrimitiveCellProps")).toBe(
      false,
    );
    expect(
    ...
    expect(primitiveContent.includes("createJsonTableDataCellProps")).toBe(
      true,
    );
  ...
  it("keeps primitive table-to-row callback boundaries stable", () => {
    ...
    const rowPolicyHookFile =
      "components/json-table/use-json-table-row-policy.ts";
    ...
    const cellShellContent = readFileSync(
      join(repoRoot, cellShellFile),
      "utf8",
    );
    ...
    expect(editSessionContent.includes("startStructuredEditSession")).toBe(
      true,
    );
    ...
    expect(rowContent.includes("jsonTableFullRenderedColumnWindow")).toBe(
      false,
    );
    ...
    expect(viewportHookContent.includes("rightPad: rightPadWidthPx")).toBe(
      true,
    );
  ...
  it("keeps json table cell modules split by responsibility", () => {
    const moduleContracts = [
      ...
      },
      {
        ...
        required: ["useJsonTableShellHandlers", "onPointerDown", "onKeyDown"],
