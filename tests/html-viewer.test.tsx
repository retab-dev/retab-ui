...
describe("FileViewerHtmlContent", () => {
  ...
  it("updates HTML filename metadata while reusing the fetched document", async () => {
    ...
    rerender(
      <FileViewer
        source={htmlUrlSource("/preview/same.html", "second.html")}
      />,
    );
  ...
  it("clamps HTML zoom between 10% and 500%", async () => {
    ...
    const { container } = render(
      <FileViewer
        source={htmlUrlSource("/samples/clamp.html", "clamp.html")}
      />,
    );
  ...
  it("does not evict active HTML preview fetches when the text cache exceeds its limit", async () => {
    ...
    const pendingByUrl = new Map(
      urls.map((url) => [url, deferred<Response>()]),
    );
    ...
    try {
      ...
      expect(
        screen
          ...
          .some((link) => link.getAttribute("download") === "broken.html"),
      ).toBe(true);
    } finally {
      consoleError.mockRestore();
    }
  ...
  it("recovers after an HTML load error when the descriptor changes", async () => {
    ...
    try {
      ...
      rerender(
        <FileViewer source={htmlUrlSource("/good.html", "good.html")} />,
      );
  ...
  it("can be mounted directly with already-loaded HTML", async () => {
    ...
    expect(fetchMock).not.toHaveBeenCalled();
    expect(
      container.querySelector('[data-slot="html-file-viewer-content"]')
        ?.className,
    ).toContain("h-full");
