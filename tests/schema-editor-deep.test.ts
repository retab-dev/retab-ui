...
describe("convert: round-trip over structural edge cases", () => {
  const cases: Array<[string, JSONSchema7]> = [
    ...
    [
      "boolean items schema",
      { type: "array", items: true } as unknown as JSONSchema7,
    ],
    [
...
describe("convert: extended fuzz round-trip", () => {
  ...
  function gen(
    ...
    if (r < 0.42 && defs.length)
      return {
        $ref: `#/$defs/${defs[Math.floor(rand() * defs.length)]}`,
      } as JSONSchema7;
    if (r < 0.5) {
    ...
    if (r < 0.58)
      return {
        anyOf: [gen(rand, depth - 1, defs), { type: "null" }],
      } as JSONSchema7;
    if (r < 0.64)
      return {
  ...
  it("preserves exact KEY ORDER for 2000 random schemas (byte-exact)", () => {
    ...
    for (let seed = 1; seed <= 2000; seed++) {
      const schema = genRoot(seed);
      if (JSON.stringify(rt(schema)) !== JSON.stringify(schema))
        failures.push(seed);
    }
    expect(failures).toEqual([]);
  });
...
function assertInvariants(doc: SchemaDocument, step: string) {
  ...
  for (const id of ids)
    expect(
      getNode(doc, id),
      `${step}: getNode could not find ${id}`,
    ).not.toBeNull();
    ...
    if (value && typeof value === "object" && !Array.isArray(value)) {
      ...
      if (Array.isArray(record.required)) {
        ...
        for (const name of record.required) {
          expect(
            seen.has(name),
            `${step}: duplicate required ${String(name)}`,
          ).toBe(false);
          seen.add(name);
        }
      }
    ...
    for (let seed = 1; seed <= 200; seed++) {
      ...
      for (let i = 0; i < 40; i++) {
        ...
        switch (op) {
          ...
          case 2:
            if (propId)
              doc = renameProperty(doc, propId, pick(["x", "y", "a", ""])!);
            break;
          case 3:
          ...
          case 8: {
            ...
            if (def)
              doc = renameDefinition(doc, def.id, pick(["Def", "Bar", "Foo"])!);
            break;
          }
          case 9: {
            const def = pick(doc.defs);
...
describe("enum index operations", () => {
  ...
  it("getEnumBaseType reflects the backing scalar type", () => {
    expect(
      getEnumBaseType(
        fromJsonSchema({ type: "number", enum: [1, 2] } as JSONSchema7).root,
      ),
    ).toBe("number");
    expect(
...
describe("object-property-edits", () => {
  it("rename preserves key order and rewrites required", () => {
    ...
    const out = renameObjectProperty({
      schemaNode: node,
      oldName: "a",
      newName: "z",
    });
  ...
  it("rename onto an existing sibling is a no-op", () => {
    ...
    expect(
      renameObjectProperty({ schemaNode: node, oldName: "a", newName: "b" }),
    ).toBe(node);
  });
...
describe("enum value input: format ⇄ parse", () => {
  ...
  for (const [name, value] of stable) {
    it(`round-trips ${name}`, () => {
      expect(parseEnumValueInput(formatEnumValueInput(value as never))).toEqual(
        value,
      );
    });
  }
  ...
  describe("BUG: string values that look like JSON literals do not round-trip", () => {
    ...
    it("documents the actual (lossy) coercions", () => {
      ...
      expect(parseEnumValueInput(formatEnumValueInput("[1,2]"))).toEqual([
        1, 2,
      ]);
      expect(parseEnumValueInput(formatEnumValueInput("  spaced  "))).toBe(
        "spaced",
      );
    });
  });
...
describe("object-template install: dependency refs are id-linked", () => {
  ...
  it("Company.address is a proper model ref to the Address definition", () => {
    ...
    const addressField = company.node.properties!.find(
      (p) => p.key === "address",
    )!;
  ...
  it("marks every object's properties required, recursively", () => {
    const out = requireAllProperties({
      ...
      $defs: {
        D: {
          type: "object",
          properties: { d1: { type: "string" }, d2: { type: "number" } },
        },
      },
      properties: {
        a: { type: "string" },
    ...
    expect(new Set(out.properties.nested.required)).toEqual(
      new Set(["x", "y"]),
    );
