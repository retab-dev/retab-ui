...
describe("DocxViewer", () => {
  ...
  it("clamps controlled scale values to the supported zoom range", async () => {
    ...
    await act(async () => {
      view.rerender(
        <DocxViewer
          source={docxUrlSource("/controlled-low.docx")}
          scale={20}
        />,
      );
    });
  ...
  it("returns to fit-width when a controlled scale prop is removed", async () => {
    ...
    const view = await renderDocx(
      <DocxViewer
        source={docxUrlSource("/controlled-to-fit.docx")}
        scale={2}
      />,
    );
  ...
  it("keeps viewer header and sidebar visible while resource bytes are loading", async () => {
    ...
    await act(async () => {
      pendingResponse.resolve(
        response(Uint8Array.of(1, 2, 3), { status: 200 }),
      );
  ...
  it("does not scroll non-document text when the imperative handle resolves targets", async () => {
    ...
    const style = document.querySelector(
      "[data-testid='docx-generated-style']",
    );
  ...
  it("clears a retained Blob read failure before retrying the same source", async () => {
    ...
    expect(
      await screen.findByText("Couldn't load this document."),
    ).toBeTruthy();
    expect(screen.getByRole("alert").getAttribute("data-error-kind")).toBe(
      "unknown",
    );
