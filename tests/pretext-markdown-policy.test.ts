...
describe("Pretext Markdown policy", () => {
  it("keeps the raw HTML sanitizer surface narrow", () => {
    ...
    expect(schema.tagNames).toContain("small")
    expect(schema.tagNames).toContain("sub")
    expect(schema.tagNames).toContain("sup")
    expect(schema.tagNames).toContain("time")
    expect(schema.tagNames).toContain("var")
    expect(schema.attributes?.abbr).toEqual(["title"])
    expect(schema.attributes?.dfn).toEqual(["title"])
    expect(schema.attributes?.time).toEqual(["dateTime", "title"])
