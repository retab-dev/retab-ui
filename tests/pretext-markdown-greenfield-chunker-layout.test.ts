...
import { getPretextMarkdownGreenfieldVisibleFrames } from "@/registry/new-york-v4/ui/pretext-markdown-greenfield-virtualizer"

function totalHeight(markdown: string, contentWidth = 820, fontScale = 1) {
  return layoutPretextMarkdownGreenfieldDocument({
    contentWidth,
    document: createPretextMarkdownGreenfieldDocument(markdown),
    fontScale,
  }).totalHeight
}
...
function frame(
  ...
  top: number,
  height: number
): PretextMarkdownGreenfieldChunkFrame {
  return {
    bottom: top + height,
    height,
    id,
...
describe("pretext markdown greenfield chunker", () => {
  it("starts a new chunk on a heading near the target size so it stays with following content", () => {
    ...
    const headingChunk = findPretextMarkdownGreenfieldChunkByBlockId(
      document,
      heading!.blockId
    )
  ...
  it("isolates a hostile block into its own chunk away from neighboring prose", () => {
    ...
    const hostileChunk = findPretextMarkdownGreenfieldChunkByBlockId(
      document,
      hostileBlock!.id
    )
...
describe("pretext markdown greenfield virtualizer visible range", () => {
  ...
  it("returns only frames intersecting the viewport with no overscan", () => {
    const visible = getPretextMarkdownGreenfieldVisibleFrames({
      frames,
      overscanPx: 0,
      scrollTop: 300,
      viewportHeight: 200,
  ...
  it("returns an empty range for an empty document", () => {
    ...
    const emptyDocument = createPretextMarkdownGreenfieldDocument("")
    const emptyFrame = layoutPretextMarkdownGreenfieldDocument({
      contentWidth: 820,
    ...
    const emptyVisible = getPretextMarkdownGreenfieldVisibleFrames({
