...
describe("inferCsvDialect", () => {
  ...
  it("prefers the file extension over the mime type", () => {
    expect(
      inferCsvDialect({ fileName: "a.csv", mimeType: "text/tab-separated-values" })
        .delimiter
    ).toBe(",")
  })
  ...
  it("infers from a URL src with query parameters", () => {
    expect(inferCsvDialect({ src: "https://x.test/a.tsv?token=1" }).delimiter).toBe(
      "\t"
    )
  })
})
...
describe("resolveCsvDialect", () => {
  ...
  it("lets an explicit hasHeader override the inferred dialect", () => {
    expect(
      resolveCsvDialect({ hasHeader: false, descriptor: { fileName: "a.csv" } })
