...
describe("normalizeTextLineRange", () => {
  ...
  it("returns null for non-finite endpoints", () => {
    expect(
      normalizeTextLineRange({ start: Number.NaN, end: 3 }, 10),
    ).toBeNull();
    expect(
      normalizeTextLineRange({ start: 1, end: Number.NaN }, 10),
    ).toBeNull();
    expect(
...
describe("resolvedTextViewerBounds", () => {
  ...
  it("names the offending bound on the error", () => {
    try {
      ...
      expect((error as TextViewerInvalidBoundsError).boundName).toBe(
        "maxBytes",
      );
    }
  });
