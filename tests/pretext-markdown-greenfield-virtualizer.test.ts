...
import type { PretextMarkdownGreenfieldChunkFrame } from "@/registry/new-york-v4/ui/pretext-markdown-greenfield-layout"
import {
...
} from "@/registry/new-york-v4/ui/pretext-markdown-greenfield-virtualizer"
...
describe("pretext markdown greenfield virtualizer", () => {
  it("anchors scroll by chunk identity instead of numeric index", () => {
    const anchor = getPretextMarkdownGreenfieldScrollAnchor({
      frames: [
  ...
  it("does not snap toward a shrunk anchored chunk's top when the offset exceeds the new height", () => {
    ...
    const restored = resolvePretextMarkdownGreenfieldScrollAnchor({
      anchor: anchor!,
