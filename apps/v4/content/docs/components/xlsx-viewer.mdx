...
- **Off-thread parse.** The workbook is parsed in a Web Worker — no conversion
  service, and no UI freeze. The worker flattens each sheet to a text blob +
  transferable typed arrays, so crossing the worker boundary costs a string clone
  rather than a clone of every cell object.
...
- **Synchronous cell reads.** Each cell is a typed-array index plus a string
  slice off the compact sheet — no per-cell objects — so switching sheets is
