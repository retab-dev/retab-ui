 ...
 */
export function autoFormatDateTimeFields(
  data: any,
  schema: any,
  path: string = "",
): any {
  ...
  // Cache for schema subtree checks to avoid repeated deep scans
  const SCHEMA_CACHE: WeakMap<object, boolean> =
    (autoFormatDateTimeFields as any)._SCHEMA_CACHE || new WeakMap();
  (autoFormatDateTimeFields as any)._SCHEMA_CACHE = SCHEMA_CACHE;

  // Detect whether a schema subtree contains any date/time format fields
  const hasDateTimeInSchema = (subSchema: any): boolean => {
    if (!subSchema || typeof subSchema !== "object") return false;
    if (SCHEMA_CACHE.has(subSchema)) return SCHEMA_CACHE.get(subSchema)!;
    ...
    try {
      ...
      SCHEMA_CACHE.set(subSchema, found);
    }
    return found;
  };
  ...
  if (Array.isArray(data)) {
    ...
    if (!schema.items || !hasDateTimeInSchema(schema.items)) {
      return data;
    }
    return data.map((item, index) =>
      autoFormatDateTimeFields(item, schema.items, `${path}[${index}]`),
    );
  }
    ...
    if (schema.properties) {
      Object.keys(schema.properties).forEach((key) => {
        if (key in result) {
          ...
          if (propertySchema.format && typeof result[key] === "string") {
