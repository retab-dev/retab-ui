...
// Local replacement for the removed computed-fields overlay DSL type. The DSL and
// its ComputedFieldCard editor have been deleted; the context still carries an
// (always-undefined) computationSpec so the public component contract is stable.
...
const unwrapSchema = (node: any, root: any) => {
  ...
  const deref = (n: any) => {
    if (n && n.$ref) {
      ...
      // Support both $defs (draft 2019-09+) and definitions (draft-07)
      if (ref.startsWith("#/$defs/")) {
        refName = ref.replace("#/$defs/", "");
        target = root?.$defs?.[refName];
      } else if (ref.startsWith("#/definitions/")) {
        refName = ref.replace("#/definitions/", "");
        target = root?.definitions?.[refName];
      }
  ...
  if (combos) {
    ...
    if (nonNull) {
      s = deref(nonNull);
      s = expandRefs(s, root?.$defs ?? root?.definitions ?? {});
    }
  }
...
function getFieldSchemaInternal(
  ...
  for (let i = 0; i < pathSegments.length; ) {
    ...
    if (
      ...
      const ref = String(currentSchema.$ref);
    ...
    if (currentSchema && currentSchema.type === "array") {
      ...
      <Tooltip>
        ...
        <TooltipContent className="max-w-xs">
          ...
          {typeof content === "string" ? (
            ...
            <pre className="max-h-64 overflow-auto rounded border bg-gray-50 p-1 text-xs leading-snug font-medium whitespace-pre-wrap text-gray-700">
          ...
          <div className="space-y-2">
            {reasonedAlternatives.map((alt, idx) => (
              <div key={idx} className="space-y-1">
                ...
                <div className="rounded border bg-gray-50 p-1 text-xs leading-snug font-medium whitespace-pre-wrap text-gray-700">
                  {alt.reasoning}
                </div>
              </div>
            ))}
...
const GroundTruthEditorInput: React.FC<GroundTruthEditorInputProps> = ({
  ...
  if (type === "object" || type === "array") {
    ...
    return (
      <Textarea
        className="h-32 w-full resize-none rounded-md border bg-gray-50 p-2 text-xs"
        value={pretty}
        onChange={(e) => {
  ...
  return (
    <Textarea
      className="h-20 w-full resize-none rounded-md border bg-gray-50 p-2 text-xs"
      value={value ?? ""}
    ...
    return (
      <TooltipProvider key={id}>
        <Tooltip>
          <TooltipTrigger asChild>
            <span
              className="inline-flex max-w-[80px] cursor-context-menu items-center gap-1 truncate rounded px-1.5 py-0.5 text-[10px] font-medium"
              style={{
          ...
          <TooltipContent side="top" className="max-w-xs">
            <div className="mb-1 text-xs text-slate-500">Ground Truth:</div>
    ...
    <div className="inline-flex items-center gap-1">
      <Popover open={isOpen} onOpenChange={handleOpenChange}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className="inline-flex max-w-[80px] cursor-pointer items-center gap-1 truncate rounded px-1.5 py-0.5 text-[10px] font-medium transition-opacity hover:opacity-80"
            style={{
              backgroundColor: colors.bg,
              color: colors.text,
              borderWidth: "1px",
        ...
        >
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-2xs font-medium text-slate-700">
                Ground Truth
              </div>
              <Button
                variant="outline"
...
const ArrayRendererItem = React.memo<ArrayRendererItemProps>(
  ({
      ...
      return (
        <div
          ...
          className={cn(
            ...
            className,
            "bg-transparent text-xs shadow-none",
          )}
        >
          <FormField
    ...
    // Complex object item: render inline with index number (no accordion)
    return (
      <div
      ...
      >
        ...
        {/* Object content column */}
        <div className="ml-1 min-w-0 flex-1 border-l border-gray-200 pl-1">
          <ObjectRenderer
            path={currentPath + "." + innerIdx}
            className={className}
            isArrayItem={true}
...
const ArrayRenderer: React.FC<ArrayRendererProps> = ({ path, className }) => {
  ...
  // Get the current data and ensure it's an array
  let arrayData = getNestedValue(form.getValues(), currentPath);

  // If the data isn't an array but should be according to schema, initialize it
  if (!Array.isArray(arrayData)) {
    arrayData = [];
    // Set the initial empty array in the form
    form.setValue(currentPath, arrayData);
  }
  ...
  return (
    <div className={cn("flex flex-row items-start gap-1")} key={path}>
      {/* Label Column - Matches PrimitiveRenderer label width */}
      <div className="flex shrink-0 items-center">
        <div
          className="group flex h-[28px] flex-row items-center gap-0.5"
          key={`${path}-header`}
        >
          <Dialog modal>
            {setSchema && showPropertyEditorPencil !== false ? (
              <DialogTrigger asChild>
                <FormLabel
                  className={cn(
                    className,
                    "mb-0 flex cursor-pointer items-center border-none bg-transparent text-sm font-medium underline-offset-4 shadow-none outline-transparent hover:bg-transparent hover:underline",
                  )}
                >
                  {label}
                </FormLabel>
              ...
              <FormLabel
                className={cn(
                  className,
                  "mb-0 flex items-center border-none bg-transparent text-sm font-medium shadow-none outline-transparent hover:bg-transparent",
                )}
              >
                {label}
              </FormLabel>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Render object items inline with index numbers */}
        {Array.isArray(arrayData) &&
        itemsSchema &&
        (itemsSchema.type === "object" ||
          itemsSchema.$ref ||
          itemsSchema.anyOf?.some(
            (o: any) => o.type === "object" || o.$ref,
          )) ? (
          <div className={cn("flex flex-col", className, "border-none")}>
        ...
        <div className="flex" key={`${path}-add-button`}>
          <div>
            <Button
              size={size}
...
const PrimitiveRenderer: React.FC<PrimitiveRendererProps> = ({
  ...
  // Local state for hover/focus interaction
  const [isHovered, setIsHovered] = React.useState(false);
  const [isInputFocused, setIsInputFocused] = React.useState(false);
  // We need to track if the popover/select is open to keep the input visible
  const [isPopoverOpen, setIsPopoverOpen] = React.useState(false);
  const activeEditedValue = resolveDraftValue(
    committedEditedValue,
    editedValue,
    isInputFocused || isPopoverOpen,
  );
        ...
        <Dialog modal>
          {setSchema && showPropertyEditorPencil !== false ? (
            <DialogTrigger asChild>
              <FormLabel
                className={cn(
                  "flex cursor-pointer items-center gap-1 underline-offset-4 hover:underline",
          ...
          {false && subschema["X-FieldPrompt"] && (
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center rounded-full border border-black px-1 text-xs font-light">
                    <Code
                      className={cn(
                        className,
                        "h-[14px] w-[14px] cursor-pointer border-none bg-transparent shadow-none outline-transparent outline-none",
  ...
  const showInput =
    (isHovered || isInputFocused || isPopoverOpen) && !isFieldDisabled;
  ...
  const handleFieldClick = () => {
    ...
    setIsInputFocused(true); // Ensure we keep input showing
      ...
      <FormField
        ...
        render={({ field: formField }) =>
          inputType === "checkbox" ? (
            <FormItem className="flex items-center space-y-2">
              <FormControl>
                <StyledCheckbox
                  ...
                  onCheckedChange={(checked) => {
                    setEditedValue(checked ? "true" : "false");
            ...
            <FormItem className="flex min-h-0 flex-1 flex-row items-center gap-3">
              {isStreaming && isUndefined(getNestedValue(formData, path))
                ? titlePosition != "none" && (
                    ...
                    >
                      <Skeleton
                        className={
                          "h-4 w-[30%] animate-pulse rounded-md border-none bg-gray-200 outline-transparent outline-none"
                        }
                      />
                    </FormLabel>
                  )
                : titlePosition != "none" && (
                    <div
                      className={cn(
                        className,
                        "flex shrink-0 items-center border-none bg-transparent text-left shadow-none outline-transparent hover:bg-transparent",
                      )}
                    >
                      <LabelContent />
                    </div>
              ...
              >
                {showInput ? (
                  <FormControl className="flex min-w-0 flex-1">
                    {isProcessing && !isStreaming ? (
                      <Skeleton
                        className={cn(
                          size === "sm" ? "h-[32px]" : "h-[42px]",
                          "flex animate-pulse rounded-md border-none bg-gray-200 outline-none",
                        )}
                      />
                        ...
                        <StyledInput
                          ...
                          className={cn(
                            ...
                            size === "sm"
                              ? "h-[28px] text-sm"
                              : "h-[28px] text-sm",
                            "text-opacity-90 border-transparent shadow-none",
                            highlightClasses(
                              isVerified,
                              isComputedField,
                              "border",
                      ...
                      <Skeleton
                        className={cn(
                          size === "sm" ? "h-[32px]" : "h-[42px]",
                          "flex animate-pulse rounded-md border-none bg-gray-200 text-red-200 outline-none",
                        )}
                      />
                    ) : fieldType === "date" ? (
                      isStreaming &&
                      isUndefined(getNestedValue(formData, path)) ? (
                        ...
                        >
                          <StyledDatePicker
                            ...
                            disabled={isFieldDisabled}
                            value={safeDate(
                              activeEditedValue || formField.value,
                            )}
                            onValueChange={(date) => {
                              ...
                              setEditedValue(validatedDate ?? undefined);
                      ...
                      isUndefined(getNestedValue(formData, path)) ? (
                        ...
                        >
                          <StyledDateTimePicker
                            ...
                            scalarValueType={scalarValueType}
                            disabled={isFieldDisabled}
                            value={activeEditedValue}
                            onValueChange={(dateTime) => {
                              setEditedValue(dateTime ?? undefined);
                  ...
                  <div
                    className={cn(
                      className,
                      "flex min-h-[28px] items-center rounded-md px-3 py-1 text-sm md:min-h-[28px]",
                    ...
                    {fieldType === "enum" ? (
                      ...
                      >
                        {(() => {
                          ...
                          try {
                            return String(formField.value).replace("T", " ");
                          } catch {
...
const ObjectRenderer: React.FC<ObjectRendererProps> = ({
  ...
  const isComputedField = Boolean(
    (subschema as any)?.["X-ComputedField"] ||
      (effectiveSchema as any)?.["X-ComputedField"],
  );
    ...
    <div
      className={cn(
        ...
        isArrayItem || isLoneObject
          ? "rounded-md border-none pt-0 pb-2 shadow-none outline-transparent"
          : "",
        showLabel ? "flex flex-row items-start gap-0 py-1" : "flex flex-col",
      )}
      key={path}
    >
      {showLabel && (
            ...
            <Dialog modal>
              {setSchema && showPropertyEditorPencil !== false ? (
                ...
                <FormLabel
                  className={cn(
                    className,
                    "mb-0 flex border-none bg-transparent text-sm font-medium shadow-none outline-transparent hover:bg-transparent",
                  )}
                >
                  {label}
                </FormLabel>
      ...
      <div
        className={cn(
          "flex min-w-0 flex-1 flex-col border-transparent",
          isLoneObject
            ? "ml-1 border-l border-gray-200 pl-1"
            : isArrayItem
              ? "border-blue-500"
              : "",
        )}
      >
        {Object.keys(properties).map((prop) => {
          ...
          const { schema: resolvedPropSchema } = unwrapSchema(
            propSchema,
            schema,
          );

          const propType =
            resolvedPropSchema?.type ||
          ...
          if (propType === "object" || resolvedPropSchema?.properties) {
            return (
              <ObjectRenderer
                key={prop}
          ...
          } else if (propType === "array" || resolvedPropSchema?.items) {
            return (
              <ArrayRenderer
                key={prop}
                path={path ? `${path}.${prop}` : prop}
        ...
        {isNullable && !disabled && (
          <Button
            ...
            className={cn(className, "mt-2 ml-auto flex")}
            disabled={disabled || isStreaming}
          >
      ...
      {Array.from({ length: numberOfFields }).map((_, index) => (
        <div key={index} className="space-y-2">
          ...
          <Skeleton
            className={cn(
              className,
              "h-10 w-full animate-pulse rounded-md border-none bg-gray-200 outline-transparent outline-none",
            )}
          />
        </div>
      ))}
...
export const UiFormContent = ({
  ...
  const { isProcessing, isStreaming } = useUiFormContext();
